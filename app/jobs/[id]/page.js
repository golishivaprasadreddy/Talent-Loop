  ...
  return (
    <main style={{ minHeight: "100vh", background: "var(--cream)" }}>
      <nav className="nav shell">
        ...
        <div className="navlinks"><a href="/">← All jobs</a><a href="/login">Sign in</a><a className="nav-button" href="/register">Get started</a></div>
      </nav>
      ...
      <div className="shell job-detail-grid" style={{ padding: "48px 0 80px", display: "grid", gridTemplateColumns: "minmax(0,1.8fr) minmax(280px,1fr)", gap: 28, alignItems: "start" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          <div style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 12, padding: 32 }}>
            ...
            {job.customQuestions?.length > 0 && (
              <div style={{ marginTop: 24, padding: "16px 20px", background: "#f7faed", border: "1px solid #ccd9a6", borderRadius: 8 }}>
                <p style={{ font: "600 13px DM Sans", margin: "0 0 4px", color: "#41531a" }}>
                  {job.customQuestions.length} screening question{job.customQuestions.length > 1 ? "s" : ""} included
                </p>
                <p style={{ fontSize: 12, color: "#6a7a50", margin: 0 }}>You'll answer them during the application process.</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="job-detail-sidebar" style={{ display: "flex", flexDirection: "column", gap: 16, position: "sticky", top: 24 }}>
          ...
          <div style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 12, padding: 24 }}>
            <p style={{ font: "600 14px DM Sans", margin: "0 0 12px" }}>About {company.name}</p>
